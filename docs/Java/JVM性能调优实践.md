...
通过jstat -gccause查看上次发生GC的原因是“Allocation Failure”，表明最近一次引起GC的原因是因为在年轻代中没有足够的空间能够存储新的数据了，综合判断来看应该是新生代容量过小导致频繁发生Minor GC；

查看Full GC发生的次数也比较频繁，一般发生Full GC是因为Eden区用满时会进行一次minor gc，将存活下来的对象复制到另外一块Survivor上，如果另一块Survivor放不下(对应虚拟机参数为XX:TargetSurvivorRatio，默认50，即50%)，对象直接进入老年代。当前Survivor分配容量过小，新生代频繁GC最后也导致老年代空间不足；

使用jmap -histo [vid]查看jvm中的class对象，发现大部分都是反射相关的实例，由于当前系统会存在导出excel文件报表的使用场景，而且导出excel文件底层逻辑的是根据导出excel工具类的配置文件对应的对象反射查询数据，然后导出成excel文件，会产生大量的sun.reflect.GeneratedMethodAccessor实例对象；
...
#查看JVM GC发生原因
[tomcat@ecc opt]# jstat -gccause 41576
  S0     S1     E      O      P     YGC     YGCT    FGC    FGCT     GCT    LGCC                 GCC                 
 20.29   0.00  57.99  50.63  34.99   292    20.926   4     0.798    21.724 Allocation Failure   No GC
```

连续观察几天，发现累计出现GC的次数太高了，Full GC的次数也过高；

```bash
#查看JVM内存使用情况 
[tomcat@ecc opt]# jstat -gcutil 41576 1000 10
  S0     S1     E      O      P     YGC     YGCT    FGC    FGCT     GCT   
  0.00  20.77  72.04  20.17  37.93   9251  764.717   175  131.130  895.847
  0.00  20.77  72.04  20.17  37.93   9251  764.717   175  131.130  895.847
  0.00  20.77  72.04  20.17  37.93   9251  764.717   175  131.130  895.847
  0.00  20.77  72.05  20.17  37.93   9251  764.717   175  131.130  895.847
  0.00  20.77  72.05  20.17  37.93   9251  764.717   175  131.130  895.847
  0.00  20.77  72.16  20.17  37.93   9251  764.717   175  131.130  895.847
  0.00  20.77  72.22  20.17  37.93   9251  764.717   175  131.130  895.847
  0.00  20.77  72.28  20.17  37.93   9251  764.717   175  131.130  895.847
  0.00  20.77  72.28  20.17  37.93   9251  764.717   175  131.130  895.847
  0.00  20.77  72.28  20.17  37.93   9251  764.717   175  131.130  895.847 
```
...
加大新生代容量，将堆初始容量和最大容量设置为相同数值

```bash
-XX:+UseConcMarkSweepGC -Xmx6144m -Xms6144m -XX:PermSize=256M -XX:MaxPermSize=512M -Xmn2048m
```
